...
**Plan 5 — Agentic commerce chatbot (roadmap, design done):** a conversational ordering agent on the storefront that knows the selected profile/loyalty/store/history + local context, recommends a personalized order via the existing `synth_qsr-recommender`, and **places a real, trackable order** through the *same* Checkout→Kafka `orders`→`order-tracker`→Valkey pipeline (returns real `order_id` / `shipping_tracking_id` / cost). Recommended design (full brainstorm in `research/agentic-commerce-chatbot_2026-06-15.md`): a Databricks-served `pizzatel-agent` MLflow `ResponsesAgent` endpoint provisioned via the existing `provisioning/` DAB (setup + destroy job), fronted by a new `pages/api/agent-chat.ts` BFF + chat widget. **Key partition:** data/model tools (recs, preferences, local_events) run on Databricks with MLflow tracing; the `place_order` action tool is *declared on the agent but executed in the BFF* by calling the real `CheckoutGateway.placeOrder` with `pizzatel-store-id`/`pizzatel-order-type` metadata — so the order is byte-identical to a UI order and never forks the pipeline. flagd `agentEnabled` kill-switch + offline fallback, mirroring `recommendationModelEnabled`. Depends on Plan 4b (model-serving wrapper) landing first; reuses the same PAT→OAuth-M2M auth trajectory (ADR 0002).

**Plan 5b — Customer deals + order-history for the agent (roadmap, v2 data path):** the deployed v1 `synth_qsr-commerce-agent` ships `search_menu`/`get_recommendations`/`get_customer_context`/`propose_order` live, but **`get_order_history` and `get_occasion_context` return empty** — so the chatbot can't yet answer "what did I order last time?" or surface personalized **deals/offers**. Follow-on: back these two tools with real data, ideally via the **customer feature store** (e.g. a `synth_qsr-customer-features`-style table for recent-order recency/frequency/last-items + an occasion/deals feature keyed on profile/loyalty tier + store + calendar/local events). Web side needs **no change** — the agent owns these tools; once the data path lands, the existing chat surfaces it. Tracked in `docs/integration/agent-endpoint-contract.md` (model-team v1 note, 2026-06-18). Surfaced during live real-agent testing (`docs/journey-test-results-2026-06-18.md`).
